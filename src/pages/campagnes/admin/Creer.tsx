...
export default function Creer() {
  ...
  return (
    <div>
      <PageHeader icon={Clapperboard} titre="Nouvelle campagne" sousTitre={options.donnees?.clientNom ?? ''} accent="#d03e0d" />
      <Card className="max-w-xl">
        <form onSubmit={envoyer} className="flex flex-col gap-3">
          <div>
            <label className="mb-1.5 block text-xs font-semibold text-gray-500">Nom de la campagne</label>
            <input
              value={nom}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1.5 block text-xs font-semibold text-gray-500">Début</label>
              <input
                type="date"
                value={dateDebut}
            ...
            <div>
              <label className="mb-1.5 block text-xs font-semibold text-gray-500">Fin</label>
              <input
                type="date"
                value={dateFin}
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold text-gray-500">Prime meilleur vendeur (optionnel)</label>
            <input
              value={primeVendeur}
              onChange={(e) => setPrimeVendeur(e.target.value)}
              placeholder="25 000"
              className="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none"
            />
          </div>
