...
export default function Commandes() {
  ...
  return (
    <div>
      <PageHeader icon={ShoppingCart} titre="Commandes" sousTitre="Suivi des commandes clients" accent="#eb6834" />

      <Card className="mb-4">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-bold text-slate-900">{idEnEdition !== null ? 'Modifier la commande' : 'Nouvelle commande'}</h2>
          {idEnEdition !== null && (
        ...
        <form onSubmit={envoyer} className="mt-3 flex items-end gap-3">
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-slate-500">Quantité 33cl</label>
            <input
              value={q33}
              onChange={(e) => setQ33(e.target.value)}
              className="w-24 rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-sm text-slate-900 focus:border-jus-primary focus:outline-none"
            />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-slate-500">Quantité 1L</label>
            <input
              value={q1l}
          ...
          <div className="flex items-center gap-2">
            {!c.est_completee &&
              (enCompletion === c.id ? (
                ...
                <>
                  <button onClick={() => setEnCompletion(c.id)} className="text-xs font-semibold text-jus-primaryDark hover:text-jus-primary">
                    Compléter →
                  </button>
                  <button
                    type="button"
      ...
      {(completion.erreur || suppression.erreur) ? (
        <p className="mt-2 text-xs font-semibold text-rose-600">{completion.erreur ?? suppression.erreur}</p>
      ) : null}
    </div>
  );
}
