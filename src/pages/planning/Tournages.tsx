...
export default function Tournages() {
  ...
  const [ideesChoisies, setIdeesChoisies] = useState<number[]>([]);
  ...
  const calendrier = useApi(
    () => calendrierTournages(periode.mois, periode.annee, filtreClient),
  ...
  function ouvrirEdition(t: NonNullable<typeof tournages.donnees>[number]) {
    ...
    setIdeesChoisies(t.content_ideas_detail.map((i) => i.id));
    setOuvert(true);
  }

  function toggleIdee(id: number) {
    setIdeesChoisies((precedent) => (precedent.includes(id) ? precedent.filter((i) => i !== id) : [...precedent, id]));
  }
  ...
  // Ouvre directement l'édition d'un tournage quand on arrive via `?edit=<id>`
  ...
  async function envoyer(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!clientId) return;
    const payload = { client: Number(clientId), date, description, content_idea_ids: ideesChoisies };
    if (idEnEdition !== null) {
  ...
  // Le sélecteur d'idées n'affiche que les 6 plus récentes en boutons rapides
  // (la liste complète — déjà une quarantaine d'entrées — passe par le menu
  // déroulant juste en dessous) ; les idées déjà liées mais plus anciennes
  // (cas d'une modification) restent visibles comme des puces à part.
  const ideesRecentes = (idees.donnees ?? []).slice(0, 6);
  const idsRecents = new Set(ideesRecentes.map((i) => i.id));
  const ideesSelectionneesHorsRecentes = (idees.donnees ?? []).filter((i) => ideesChoisies.includes(i.id) && !idsRecents.has(i.id));
                    ...
                    >
                      ...
                      <div className="mt-1 space-y-1">
                        {jour.tournages.map((t) => (
                          <div
                            key={t.id}
                            title={`${t.client_nom} — ${LIBELLES_STATUT[t.status]}`}
          ...
          <Card className="w-full max-w-lg !bg-surface">
            ...
            <form onSubmit={envoyer} className="flex flex-col gap-3">
              <div>
                ...
                <select value={clientId} onChange={(e) => setClientId(e.target.value)} required className={CHAMP}>
              ...
              <div>
                <label className={LABEL}>Idées de contenu liées</label>
                <div className="flex flex-wrap gap-1.5">
                  {ideesRecentes.map((idee) => (
                    <button
                      type="button"
                      key={idee.id}
                      onClick={() => toggleIdee(idee.id)}
                      className={`rounded-lg border px-2.5 py-1.5 text-xs font-semibold ${
                        ideesChoisies.includes(idee.id) ? 'border-accent bg-accent/20 text-accent2' : 'border-border bg-surface2 text-muted'
                      }`}
                    >
                      {idee.titre}
                    </button>
                ...
                {ideesSelectionneesHorsRecentes.length > 0 && (
                  <div className="mt-1.5 flex flex-wrap gap-1.5">
                    {ideesSelectionneesHorsRecentes.map((idee) => (
                      <button
                        type="button"
                        key={idee.id}
                        onClick={() => toggleIdee(idee.id)}
                        className="rounded-lg border border-accent bg-accent/20 px-2.5 py-1.5 text-xs font-semibold text-accent2"
                      >
                        {idee.titre} ×
                      </button>
                    ))}
                ...
                >
                  <option value="">+ Ajouter une autre idée...</option>
                  {(idees.donnees ?? [])
                    .filter((idee) => !ideesChoisies.includes(idee.id))
                    .map((idee) => (
                      <option key={idee.id} value={idee.id}>
                        {idee.titre}
                      </option>
                    ))}
                </select>
